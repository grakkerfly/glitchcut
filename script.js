            ...
            return new Promise(resolve=>{
                if(media.type==='image'){
                    ...
                    img.onload=function(){
                        ...
                        imgElement.src=media.src; imgElement.alt='';
                    ...
                    videoForThumb.src=media.src; videoForThumb.muted=true; videoForThumb.preload='auto'; videoForThumb.style.display='none'; document.body.appendChild(videoForThumb);

                    videoForThumb.addEventListener('loadedmetadata',()=>{ videoForThumb.currentTime=0.1; });
                    videoForThumb.addEventListener('seeked',()=>{
                        try{
                            const canvas=document.createElement('canvas');
                            canvas.width=videoForThumb.videoWidth; canvas.height=videoForThumb.videoHeight;
                        ...
                        }finally{ videoForThumb.remove(); }
                    });

                    videoForThumb.load();

                    const playIcon=document.createElement('div');
                    playIcon.className='video-play-icon'; playIcon.innerHTML='▶';
                    playIcon.style.position='absolute'; playIcon.style.top='50%'; playIcon.style.left='50%'; playIcon.style.transform='translate(-50%,-50%)';
                    playIcon.style.fontSize='40px'; playIcon.style.color='white'; playIcon.style.textShadow='0 0 10px rgba(0,0,0,0.7)'; playIcon.style.zIndex='10';
    ...
    function startBounceAnimation(element,index){
        ...
        function animate(){
            ...
            if(newX<=0||newX>=window.innerWidth-rect.width){
                speedX=-speedX; element.style.filter='brightness(500%)'; setTimeout(()=>{element.style.filter='';},50);
            }
            if(newY<=0||newY>=window.innerHeight-rect.height){
                speedY=-speedY; element.style.filter='brightness(500%)'; setTimeout(()=>{element.style.filter='';},50);
            }
